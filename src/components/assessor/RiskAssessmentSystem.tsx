...
export default function RiskAssessmentSystem(): JSX.Element {
    ...
    try {
      ...
      try {
        ...
        if (
          ...
          errorMessage.includes("Incorrect time range")
        ) {
                                  ...
                                  >
                                    <div className="flex items-center gap-2 min-w-0 flex-1">
                                      ...
                                      <div className="min-w-0">
                                        ...
                                        {(farm.locationName || (farm.province && farm.district ? `${farm.province}, ${farm.district}` : '') || farm.province || farm.district) && (
                                          <p className="text-[10px] text-gray-400 flex items-center gap-1 mt-0.5 max-w-[220px] truncate">
                                            <MapPin className="h-3 w-3 text-gray-400 shrink-0" />
                                            <span>{farm.locationName || `${farm.province}, ${farm.district}`}</span>
                                          </p>
                                        )}
                                      </div>
                                    </div>
  ...
  const computedRisk = assessment ? calculateOverallRisk(assessment.dronePdfs || [], assessment.weatherData) : null;
  const finalRiskScore = riskScore !== null && riskScore !== undefined && typeof riskScore === "number"
    ? riskScore
    : (computedRisk ? computedRisk.score : null);
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-green-900">
                    {finalRiskScore !== null && finalRiskScore !== undefined && typeof finalRiskScore === "number"
                      ? finalRiskScore.toFixed(1)
                      : "N/A"}
                  </p>
                </div>
          ...
          >
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                <div>
                  <p className={`text-xs font-medium mb-1 ${
                    assessment.reportGenerated ? "text-emerald-600" : "text-gray-500"
                  }`}>
                    Report
                  </p>
                  <p className={`text-lg font-bold ${
                    assessment.reportGenerated ? "text-emerald-900" : "text-gray-900"
                  }`}>
            ...
            <BasicInfoTab
              ...
              season={
                ...
                (selectedFarm?.sowingDate
                  ? (() => {
                      const m = new Date(selectedFarm.sowingDate).getMonth();
                      return m >= 8 || m <= 0 ? "A" : "B";
                    })()
                  : "B")
              }
