    ...
    try {
      ...
      if (response.status === 500) {
        ...
        if (errorDetail.includes("Cannot read properties of null")) {
          throw new Error(
            "Server error: Missing data in database. Please contact support or try again later.",
          );
        }
                ...
                if (Array.isArray(messages)) {
                  messages.forEach((msg: any) => {
                    validationMessages.push(
                      `${field}: ${msg.message || msg.msg || msg}`,
                    );
                  });
                } else if (typeof messages === "object" && messages !== null) {
                  validationMessages.push(
                    `${field}: ${(messages as any).message || JSON.stringify(messages)}`,
                  );
                } else {
      ...
      if (
        ...
        throw new Error(
          `Request timed out after ${timeoutSeconds} seconds. The server may be processing your request (e.g., sending notifications). Please wait a moment and check if the operation completed, or try again.`,
        );
      }
...
// Endpoint: POST /assessments/:id/upload-drone-pdf?pdfType=plant_health(or flowering)
async uploadDronePDF(
  ...
  const url = `${this.baseURL}/${assessmentId}/upload-drone-pdf?pdfType=${pdfType}`;
  ...
  async deleteDronePDF(assessmentId: string) {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        errorData.message ||
          errorData.error ||
          `Failed to delete drone PDF: ${response.status} ${response.statusText}`,
      );
    }
  ...
  // Delete Specific PDF by Type (Assessor Only)
  // Endpoint: DELETE /assessments/:id/pdfs/:pdfType
  async deleteAssessmentPdf(assessmentId: string, pdfType: string) {
    console.log("🗑 Deleting assessment PDF:", { assessmentId, pdfType });

    try {
      const response = await this.request(`/${assessmentId}/pdfs/${pdfType}`, {
        method: "DELETE",
      });
