      ...
      if (!response.ok) {
        ...
        if (response.status === 401) {
          ...
          const conflictMessage =
            data.message || data.error || "User already exists.";
...
export const deactivateUser = (userId: string) =>
  usersApiService.deactivateUser(userId);
...
export const getAssessors = (page: number = 0, size: number = 100) =>
  usersApiService.getAssessors(page, size);
export const getInsurers = (page: number = 0, size: number = 100) =>
  usersApiService.getInsurers(page, size);
