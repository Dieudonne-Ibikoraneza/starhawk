...
class PoliciesApiService {
      ...
      if (!response.ok) {
        ...
        // Format 3: Validation errors in different fields (common in Spring Boot)
          ...
          if (Array.isArray(data.validationErrors)) {
            validationErrors.push(
              ...data.validationErrors.map((e: any) => e.message || e),
            );
          } else {
            Object.entries(data.validationErrors).forEach(
              ([field, msg]: [string, any]) => {
  ...
  async createPolicyFromAssessment(
    assessmentId: string,
    coverageLevel: "BASIC" | "STANDARD" | "PREMIUM",
    startDate: string,
    endDate: string,
...
export const createPolicy = (policyData: PolicyData) =>
  policiesApiService.createPolicy(policyData);
export const createPolicyFromAssessment = (
  assessmentId: string,
  coverageLevel: "BASIC" | "STANDARD" | "PREMIUM",
  startDate: string,
  endDate: string,
  ...
  policiesApiService.createPolicyFromAssessment(
    assessmentId,
    coverageLevel,
    startDate,
    endDate,
...
export const getPolicies = (page?: number, size?: number, status?: string) =>
  policiesApiService.getPolicies(page, size, status);
export const getPolicyById = (id: string) =>
  policiesApiService.getPolicyById(id);
